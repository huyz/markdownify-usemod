...
const USEMOD_URL_REGEX = /^\?([^=]+)$/;
// For links to non-existent pages, usemod creates links to ?action=edit&id=LibOop
// but we'll just output the same format as for existent links.
const USEMOD_ANCHORS_REGEX = /(\[[^\]]*\]\s?\()\?(?:action=edit&id=)?([^\)]+)(\))/g;
...
module.exports = async (urls, cmd) => {
    ...
    while (urls.length > 0) {
        ...
        if (options.crawl) {
            for (let i = 0; i < anchors.length; i++) {
                try {
                    ...
                    if (href && aurl.host === url.host && USEMOD_URL_REGEX.test(aurl.search) &&
                      aurl.href in doneURLS === false) {
                        console.debug(`Adding: ${aurl.href} ${href}`);
                        urls.push(aurl);
        ...
        if (root) {
            try {
                let markdown = turndown(root) || '';
                markdown = markdown.replace(USEMOD_ANCHORS_REGEX, '$1$2.md$3');
                ...
                console.error('Error while parsing markdown on page ', url.href, ': ', err);
                continue;
            }
        }
    }
    ...
    if (options.out) {
        for (let i = 0; i < pages.length; i++) {
            try {
                ...
                if (pathname === '/') {
                  const search_match = search.match(USEMOD_URL_REGEX);
                  if (search_match) {
                    // If this is a link to an internal UseMod page
                    pathname = `/${search_match[1]}.md`;
                  } else {
                    pathname = '/index.md';
                  }
                } else {
